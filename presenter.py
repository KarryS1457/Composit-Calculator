...
class AppPresenter:
    ...
    def _validate_geometry(self, raw_data, item_type=None):
        ...
        # Внутренний диаметр не может достать до наружного: у кольца тогда
        # не остается стенки, и расчет молча выдавал число по отрицательным
        # длинам. Проверяем до всех остальных сверок с заготовкой.
        if D > 0 and d > 0 and d >= D:
            return ("ОШИБКА: Внутренний диаметр детали (d) не может быть больше "
                    "или равен внешнему диаметру детали (D).")
