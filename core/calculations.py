...
def get_AWC_coeff(target_d, target_s):
    ...
    Значение берется СТУПЕНЬКОЙ — ближайший узел таблицы снизу, как в
    эталонном Excel (B91, приблизительный HLOOKUP).

    В ветке beta вместо этого стоит билинейная интерполяция между четырьмя
    ближайшими узлами: ступенька дает разрыв на границе диапазона, из-за
    которого деталь большего диаметра может нормироваться дешевле меньшей
    (Ø290 — 713 сек, Ø300 — 635 сек).

    Здесь интерполяции нет намеренно. Она вскрывает аномальные ячейки
    таблицы: например 0.91 при D1=400-499 и S=29-38 — минимальное значение
    во всей таблице при соседях 1.27-1.75. Ступенька держала это значение на
    весь диапазон, интерполяция усредняет его с соседями, и ходовой
    типоразмер (фланцы Ø440-480 из листа 30-36) дорожает вдвое. Пока не
    решено, опечатка это или реальная норма, расчет остается прежним.
    ...
    return float(data.AWC_DATA[row_key][col_idx])
...
def calculate_lathe_time(item_type, p, m_info=None, force_machine=None):
    ...
    if force_machine:
        ...
        # В эталонном Excel (B78) выбор идет по D, но это неверно: в патрон
        # зажимается заготовка, и станок должен принять именно ее. При выборе
        # по D деталь с D чуть ниже границы диапазона, но с заготовкой уже за
        # границей, попадала на станок, который физически ее не держит.
        #
        # Берем первый станок, чей ВЕРХНИЙ предел не меньше диаметра, перебирая
        # их от меньшего к большему — та же логика, что и вложенные
        # IF(>250; IF(>500; ...)) в B78. Сравнивать с нижней границей нельзя:
        # в нормах диапазоны заданы целыми (0-250, 251-500, 501-800, 801-2500),
        # и проверка "low <= D <= high" теряла дробные диаметры в стыках —
        # 250.5, 500.5 и 800.5 не попадали ни в один диапазон, и расчет
    ...
    if item_type in TABLE_TYPES:
        ...
        # B122: внешняя проточка по габариту t (F18/B106) — Excel добавляет ее
        # ВСЕМ типам изделий.
        #
        # ВНИМАНИЕ, как и B123 ниже, строка выглядит лишней: это то же наружное
        # продольное точение, что и B111 — те же проходы B80 по тому же
        # припуску (D1-D)/2 и та же скорость B55, отличается только длина
        # (габарит t вместо толщины заготовки S, а t > S не пропускает
        # валидация). Однажды она отсюда убиралась как явный дубль.
        #
        # Ее вернули по той же причине, что и B123: нормы выводились с замерами
        # работы токаря, и замерами выверен ИТОГ целиком, со всеми слагаемыми.
        # Выбирать, какие из странных формул оставить, а какие убрать, по их
        # внешнему виду нельзя: когда убрали только B122, разные группы деталей
        # разъехались — типам с параметром a не хватало 11% до факта, типам без
        # него 23%, и одним коэффициентом это не сводится.
        B122 = (t * B80) / speed_long if speed_long > 0 else 0

        # B123: проточка по глубине a на глубину (D-Dc)/2 (F19/F20/B88).
        #
        # ВНИМАНИЕ, строка выглядит неверной, но оставлена намеренно.
        # Второй наружный диаметр Dc есть только у штифта; у остальных типов
        # IFERROR в F20 подставляет ноль, и формула считает съем "до нулевого
        # диаметра" — для фланца D=1170 это 585 мм на сторону и 98 проходов.
        # Физического смысла в этом нет, и однажды строка отсюда убиралась.
        #
        # Ее вернули, потому что нормы выводились с замерами работы токаря на
        # станке, и замеры подтверждали именно такой уровень времени: у типов
        # с параметром a эта строка дает около половины машинного времени и
        # задает его рост с глубиной проточки. Без нее расчет становится
        # осмысленным по форме, но расходится с фактом в 2.5-2.8 раза, причем
        # множитель зависит от a (от 1.15x при a=0 до 5.3x при a=40) и потому
        # не выражается никаким коэффициентом: у таблицы вспомогательных работ
        # оси D1 и S, параметра a там нет.
        #
        # То есть строка численно подменяет собой работу, которую модель не
        # учитывает иначе. Убирать ее можно только вместе с переносом этой
        # работы в явное слагаемое, выверенное по тем же замерам.
        B123 = 0
        if a > 0:
    ...
    steps = []
    steps.append(f"Станок: {current_machine} (по диаметру заготовки D1={final_D1:g} мм)")
    ...
    return {
        ...
        # Диаметр заготовки, по которому выбран станок: нужен вызывающему
        # коду, чтобы подбирать альтернативные станки по тому же признаку
        # (D1 может считаться здесь из припуска, снаружи его не знают).
        "blank_D1": final_D1,
