...
Коэффициент вспомогательных работ здесь берется ступенькой, как в B91.
В ветке beta вместо нее интерполяция — см. комментарий у get_AWC_coeff().
"""
import math
import os
import random
...
def excel_total(ptype, p):
    ...
    # B122: внешняя проточка по габариту t — для ВСЕХ типов, как в .xlsm.
    ops += (t * B80) / B55 if B55 > 0 else 0
    # B123: проточка по глубине a, глубина проходов (D-Dc)/2 (без Dc -> D/2).
    # Как в .xlsm: строка держит уровень норм, выверенный по замерам.
    if a > 0:
