...
class shaft(tk.Frame):
    def __init__(self, parent, presenter):
        ...
        self.ent_Dc1 = self.add_entry(left_frame, "Диаметр проточки Dc1 (мм):")
        self.ent_c1 = self.add_entry(left_frame, "Длина проточки c1 (мм):")
        self.ent_Dm1 = self.add_entry(left_frame, "Диаметр проточки Dm1 (мм):")
        self.ent_m1 = self.add_entry(left_frame, "Длина проточки m1 (мм):")
        self.ent_Dc2 = self.add_entry(left_frame, "Диаметр проточки Dc2 (мм):")
        self.ent_c2 = self.add_entry(left_frame, "Длина проточки c2 (мм):")
        self.ent_Dm2 = self.add_entry(left_frame, "Диаметр проточки Dm2 (мм):")
        self.ent_m2 = self.add_entry(left_frame, "Длина проточки m2 (мм):")
        ...
        self.set_image(resource_path("pics/shaft.png")) # Путь относительно корня проекта
    ...
    def run_calculation(self):
        try:
            payload = {
                ...
                "Dm1": float(self.ent_Dm1.get() or 0),
                "m1": float(self.ent_m1.get() or 0),
                "Dc2": float(self.ent_Dc2.get() or 0),
                "c2": float(self.ent_c2.get() or 0),
                "Dm2": float(self.ent_Dm2.get() or 0),
                "m2": float(self.ent_m2.get() or 0),
                "Da": float(self.ent_Da.get() or 0),
                "a": float(self.ent_a.get() or 0),
                "D1": float(self.ent_D1.get() or 0),
                "S": float(self.ent_S.get() or 0),
            }
            # Отправляем в презентер с типом "shaft"
            self.controller.handle_lathe_calculation("shaft", payload)
        except ValueError:
